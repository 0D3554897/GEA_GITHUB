...
REM if the file exists then run it:
    if exist Z:\IMAPS_DATA\Interfaces\PROGRAMS\batch\INTERFACE.BAT (
        CALL Z:\IMAPS_DATA\Interfaces\PROGRAMS\batch\INTERFACE.BAT
        rem exit /B 0
    ) else (
...
set LIBS_HOME=Z:\IMAPS_Data\Interfaces\PROGRAMS\java\LIBS\
SET DBG=1
