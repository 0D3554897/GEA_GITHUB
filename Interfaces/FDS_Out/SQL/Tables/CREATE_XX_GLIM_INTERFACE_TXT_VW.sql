...
/****** Object:  View [dbo].[XX_GLIM_INTERFACE_TXT_VW]    Script Date: 09/13/2018 11:14:59 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[XX_GLIM_INTERFACE_TXT_VW]'))
...
/****** Object:  View [dbo].[XX_GLIM_INTERFACE_TXT_VW]    Script Date: 09/13/2018 11:15:00 ******/
SET ANSI_NULLS ON
GO
...
/* 
NOT Used by CFF for GLIM Interface, however
This view has same results, except amounts are not OVERPUNCHED
...
,space(2) as FILLER3
,case when AVG(a.INVC_AMT) - AVG(a.CSP_AMT) < 0 then left(ltrim(cast(cast(AVG(A.INVC_AMT*100) as int)- cast(AVG(CSP_AMT*100) as int) as varchar(25))),1) else '0' end  + right('000000000000000' + ltrim(cast(abs(cast(AVG(A.INVC_AMT*100) as int)- cast(AVG(CSP_AMT*100) as int)) as varchar(25))),15) as AMOUNTLOCALCURRENCY
...
,month(GETDATE()) as ACCOUNTINGMONTHLOCAL
,space(2) as FILLER3
,case when sum(B.SALES_TAX_AMT)*-1 < 0 then left(ltrim(cast(sum(B.SALES_TAX_AMT)*-1 as varchar(25))),1) else '0' end  + right('000000000000000' + ltrim(cast(cast((abs(sum(B.SALES_TAX_AMT)*-100)) as int) as varchar(25))),15) as AMOUNTLOCALCURRENCY
,'000000000000000' as ZERO
,space(7) as FILLER4
,left(right(A.INVC_ID,7)+'           ',10) as INVOICENUMBER
,space(2) as FILLER5
...
,month(GETDATE()) as ACCOUNTINGMONTHLOCAL
,space(2) as FILLER3
,case when sum(B.BILLED_AMT - B.SALES_TAX_AMT)*-1 < 0 then left(ltrim(cast(sum(B.BILLED_AMT - B.SALES_TAX_AMT)*-1 as varchar(25))),1) else '0' end  + right('000000000000000' + ltrim(cast(cast((abs(sum(B.BILLED_AMT - B.SALES_TAX_AMT)*-100)) as int) as varchar(25))),15) as AMOUNTLOCALCURRENCY
,'000000000000000' as ZERO
,space(7) as FILLER4
,left(right(A.INVC_ID,7)+'           ',10) as INVOICENUMBER
,space(2) as FILLER5
