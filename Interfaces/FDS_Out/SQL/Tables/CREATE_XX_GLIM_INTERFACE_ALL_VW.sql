...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[XX_GLIM_INTERFACE_ALL_VW]'))
...
/****** Object:  View [dbo].[XX_GLIM_INTERFACE_ALL_VW]    Script Date: 09/13/2018 11:15:00 ******/
SET ANSI_NULLS ON
GO
...
,space(2) as FILLER3
,IMAPSSTG.DBO.XX_NEG_OVERPUNCH_UF(case when AVG(a.INVC_AMT) - AVG(a.CSP_AMT) < 0 then left(ltrim(cast(cast(AVG(A.INVC_AMT*100) as bigint)- cast(AVG(CSP_AMT*100) as bigint) as varchar(25))),1) else '0' end  + right('000000000000000' + ltrim(cast(abs(cast(AVG(A.INVC_AMT*100) as bigint)- cast(AVG(CSP_AMT*100) as bigint)) as varchar(25))),15)) as AMOUNTLOCALCURRENCY
