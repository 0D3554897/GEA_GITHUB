...
/****** Object:  View [dbo].[XX_GLIM_INTERFACE_ALL_VW]    Script Date: 3/15/2022 2:36:27 PM ******/
...
SELECT  
	...
	FROM ( 
		...
		select   
			...
			SPACE(4) AS PROJECT,
			space(3) as IGSPROJECTSECONDARYAUDITMUSTBE,
		...
		select  
			...
			'202' as MAJOR,
			'0112' as MINOR,
			'0080' as SUBMINORMANDATORYINLEADING,
			'FED   ' as LUNIT ,space(28) as FILLER1,
			'121' as LEDGERSOURCE,
			'TBD' as ACCOUNTANT,
			'FED    ' as INDEXNUMBERVOUCHERNUMBER,
			space(5) as FILLER2,
			...
			right('000' + cast(month(GETDATE()) as varchar),2) as ACCOUNTINGMONTHLOCAL,
			space(2) as FILLER3,
			IMAPSSTG.DBO.XX_NEG_OVERPUNCH_UF(case when sum(B.SALES_TAX_AMT)*-1 < 0 then left(ltrim(cast(sum(B.SALES_TAX_AMT)*-1 as varchar(25))),1) else '0' end  + right('000000000000000' + ltrim(cast(cast((abs(sum(B.SALES_TAX_AMT)*-100)) as bigint) as varchar(25))),14)) as AMOUNTLOCALCURRENCY,
			'000000000000000' as ZERO,
			space(7) as FILLER4 ,
			left(right(A.INVC_ID,7)+'           ',10) as INVOICENUMBER,
			space(2) as FILLER5,
		...
		/*'DTL' as ID ,*/ 
			...
			right('000' + cast(month(GETDATE()) as varchar),2) as ACCOUNTINGMONTHLOCAL ,
			space(2) as FILLER3 ,
			IMAPSSTG.DBO.XX_NEG_OVERPUNCH_UF(case when sum(B.BILLED_AMT - B.SALES_TAX_AMT)*-1 < 0 then left(ltrim(cast(sum(B.BILLED_AMT - B.SALES_TAX_AMT)*-1 as varchar(25))),1) else '0' end  + right('000000000000000' + ltrim(cast(cast((abs(sum(B.BILLED_AMT - B.SALES_TAX_AMT)*-100)) as bigint) as varchar(25))),14)) as AMOUNTLOCALCURRENCY,
			'000000000000000' as ZERO ,
			space(7) as FILLER4 ,
			left(right(A.INVC_ID,7)+'           ',10) as INVOICENUMBER ,
			space(2) as FILLER5 ,
