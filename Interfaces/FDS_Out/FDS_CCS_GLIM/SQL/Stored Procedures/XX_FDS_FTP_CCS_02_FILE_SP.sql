USE IMAPSSTG
DROP PROCEDURE [dbo].[XX_FDS_FTP_CCS_02_FILE_SP]
GO
SET ANSI_NULLS ON 
GO
...
CREATE PROCEDURE [dbo].[XX_FDS_FTP_CCS_02_FILE_SP]
(
...
AS
/************************************************************************************************  
Name:       	XX_FDS_FTP_CCS_02_FILE_SP  
Author:     	CR, KM
Created:    	08/2018  
Purpose:    	FTP INVOICE FILE TO CCS_02
...
CR10364 - ADDED NEW CONTROL POINT FOR FTP OF CCS_02 FILE
************************************************************************************************/  
...
PRINT '*~^**************************************************************************************************************'
PRINT '*~^                                                                                                             *'
PRINT '*~^                        HERE IN XX_FDS_FTP_CCS_02_FILE_SP.sql'
PRINT '*~^                                                                                                             *'
PRINT '*~^**************************************************************************************************************'
PRINT '' --CR9449 *~^
-- *~^
SET @SP_NAME = 'XX_FDS_FTP_CCS_02_FILE_SP'
...
PRINT convert(varchar, current_timestamp, 21) + ' : *~^ FDSCCS : Line 79 : XX_FDS_FTP_CCS_02_FILE_SP.sql '  --CR9449
...
PRINT convert(varchar, current_timestamp, 21) + ' : *~^ FDSCCS : Line 138 : XX_FDS_FTP_CCS_02_FILE_SP.sql '  --CR9449
...
PRINT convert(varchar, current_timestamp, 21) + ' : *~^ FDSCCS : Line 148 : XX_FDS_FTP_CCS_02_FILE_SP.sql '  --CR9449
...
PRINT convert(varchar, current_timestamp, 21) + ' : *~^ FDSCCS : Line 158 : XX_FDS_FTP_CCS_02_FILE_SP.sql '  --CR9449
...
SELECT @TRX_FILE = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
   AND PARAMETER_NAME = 'CCS_02_TRX_FILE'

SELECT @TEXT_FILE = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
   AND PARAMETER_NAME = 'CCS_02_JAVA_TEXT_FILE'

SELECT @FTP_FILE = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
   AND PARAMETER_NAME = 'CCS_02_FTP_FILE'
...
BEGIN
   ...
   */
   ...
   IF @rtn_code <> 0 OR @@ERROR <> 0
      BEGIN
        PRINT 'FAILURE: CCS_02 DATA FILE NOT FOUND'
        RETURN (557)
      END
   
   PRINT 'COPY CCS_02 TEXT DATA FILE' 
   ...
   IF @rtn_code <> 0 OR @@ERROR <> 0
      BEGIN
        PRINT 'FAILURE: CCS_02 TEXT DATA FILE NOT FOUND'
        RETURN (557)
      END

/******************** there is no transaction file   ***********************	
   PRINT 'COPY CCS_02 TRX FILE'
   SET @CMD = 'COPY /Y ' + @FTP_DIR + @TRX_FILE + ' ' + @ARCHIVE_DIR + CAST(@in_STATUS_RECORD_NUM as varchar(10)) + '_' + @TRX_FILE
   PRINT @CMD  
   EXEC @rtn_code = master.dbo.xp_cmdshell @CMD
   IF @rtn_code <> 0 OR @@ERROR <> 0
        BEGIN
        PRINT 'FAILURE: CCS_02 TRX FILE NOT FOUND'
        RETURN (557)
      END
*****************************************************************************/
   PRINT 'MOVE CCS_02 JAVA LOG'
     
   SET @CMD = 'MOVE ' + @CFF_LOG + ' ' + @ARCHIVE_DIR + CAST(@in_STATUS_RECORD_NUM as varchar(10)) + '_' + substring(@CFF_LOG,CHARINDEX('CFF_CCS_02.TXT',@CFF_LOG),99)
   PRINT @CMD
   EXEC @rtn_code = master.dbo.xp_cmdshell @CMD
   IF @rtn_code <> 0 OR @@ERROR <> 0
       BEGIN
        PRINT 'FAILURE: CCS_02 JAVA LOG FILE NOT FOUND'
        RETURN (557)
      END
      

   PRINT 'CCS_02 INTERFACE FILE ARCHIVED BUT NOT SENT'
END
...
PRINT convert(varchar, current_timestamp, 21) + ' : *~^ FDSCCS : Line 261 : XX_FDS_FTP_CCS_02_FILE_SP.sql '  --CR9449

   SELECT @FTP_CMD = PARAMETER_VALUE
     FROM XX_PROCESSING_PARAMETERS
    WHERE INTERFACE_NAME_CD = 'FDS'
      AND PARAMETER_NAME = 'CCS_02_FTP_CMD'
...
PRINT convert(varchar, current_timestamp, 21) + ' : *~^ FDSCCS : Line 271 : XX_FDS_FTP_CCS_02_FILE_SP.sql '  --CR9449
   ...
   SELECT @FTP_log_file = PARAMETER_VALUE
	...
	PRINT 'CCS_02_FTP_LOG_DIR IS ' + @FTP_LOG_DIR
	PRINT 'CCS_02_FTP_LOG_FILE IS ' + @FTP_LOG_FILE
	PRINT 'JAVA_LOG_DIR IS ' + @JAVA_LOG_DIR

    SELECT @FTP_ini_file = PARAMETER_VALUE
     FROM XX_PROCESSING_PARAMETERS
    WHERE INTERFACE_NAME_CD = 'FDS'
      AND PARAMETER_NAME = 'CCS_02_FTP_INI_FILE'
   ...
   SELECT @FTP_ADJUST = CAST(PARAMETER_VALUE AS INT)
    ...
    WHERE INTERFACE_NAME_CD = 'FDS'
      AND PARAMETER_NAME = 'CCS_02_FTP_ADJUST'

   -- SEARCH PHRASE
   SELECT @SEARCH_PHRASE = PARAMETER_VALUE
     FROM IMAPSSTG.DBO.XX_PROCESSING_PARAMETERS
    WHERE INTERFACE_NAME_CD = 'FDS'
      AND PARAMETER_NAME = 'CCS_02_SEARCH_PHRASE'  
...
-- END FTP TASK
   ...
   IF @rtn_code <> 0 OR @@ERROR <> 0
     BEGIN
       PRINT 'CCS_02 FTP LOG NOT ARCHIVED.'
       -- RETURN (@rtn_code)  
     END
   PRINT 'CCS_02 FTP LOG ARCHIVED'
...
-- DELETE THE FTP SUCCESS OR FAILURE FILE FROM THE FTP FOLDER
 ...
 -- OLD WAY, USING MACRO SCHEDULER
   ...
   IF @rtn_code <> 0 OR @@ERROR <> 0
      BEGIN
        PRINT 'CCS_02 FTP EVALUATION ERROR! CODE RETURNED = '  + cast(@rtn_code as varchar)
        RETURN (@rtn_code)
        -- RETURN(557)
      END
*******************************************************************************************************************/
...
-- END FTP TASK
  ...
  PRINT @CMD
   ...
   IF @rtn_code = 0 AND @@ERROR = 0
     BEGIN
       PRINT 'CCS_02 REQUIRED FTP CHECK INI FILE NOT SPECIFIED.'
       RETURN (558)  
     END
-- END IF   
  ...
  PRINT @CMD
   ...
   IF @rtn_code = 0 AND @@ERROR = 0
     BEGIN
       PRINT 'CCS_02 REQUIRED FTP CHECK SEARCH FILE NOT SPECIFIED.'
       RETURN (559)  
     END
-- END IF   
  ...
  PRINT @CMD
   ...
   IF @rtn_code = 0 AND @@ERROR = 0
     BEGIN
       PRINT 'CCS_02 REQUIRED FTP CHECK SEARCH STRING NOT SPECIFIED.'
       RETURN (560)  
     END
-- END IF 
  ...
  PRINT @CMD
   ...
   IF @rtn_code = 0 AND @@ERROR = 0
     BEGIN
       PRINT 'CCS_02 REQUIRED FTP CHECK SEARCH COUNT NOT SPECIFIED.'
       RETURN (561)  
     END
-- END IF 
  ...
  PRINT @CMD
   ...
   IF @rtn_code = 0 AND @@ERROR = 0
     BEGIN
       PRINT 'CCS_02 FTP LOG SHOWS WRONG NUMBER OF FILES TRANSFERRED'
       RETURN (562)  
     END
-- END IF 
...
-- END IF 
   ...
   PRINT 'CCS_02 INTERFACE FILE SENT'
END
...
PRINT '*~^*************************************************************************************************************'
PRINT '*~^                                                                                                             *'
PRINT '*~^                        END OF  XX_FDS_FTP_CCS_02_FILE_SP.sql'
PRINT '*~^                                                                                                             *'
PRINT '*~^**************************************************************************************************************'
PRINT '' --CR9449 *~^
RETURN(0)
