...
SELECT @FDS_OUTPUT_PATH = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
   AND PARAMETER_NAME = 'FDS_OUTPUT_PATH' 

SELECT @CCS_OUTPUT_PATH = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
...
SET @CMD = @JAVA_EXE + ' ' + @JAVA_CLASS + ' ' + @SERVER + ' ' + @FDS_OUTPUT_PATH + 'FDS\FDS\'
-- CR-9449 End
...
SELECT @CCS_02_EXE = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
...
SET @CMD = @CCS_02_EXE +' ' + @CCS_02_CFF_FILES  + ''
...
PRINT 'NOW CHECKING CCS_02 FILE'
...
SELECT @GLIM_EXE = PARAMETER_VALUE
  FROM dbo.XX_PROCESSING_PARAMETERS
 WHERE INTERFACE_NAME_CD = 'FDS'
...
PRINT 'NOW BUILDING CFF COMMAND'
SET @CMD = @GLIM_EXE +' ' + @GLIM_CFF_FILES  + ''
PRINT 'CFF COMMAND IS : ' + @CMD
EXEC @ret_code = master.dbo.xp_cmdshell @CMD  
...
-- DOES OUTPUT FILE EXIST?
PRINT 'NOW CHECKING GLIM EBCDIC FILE'
...
PRINT 'GLIM EBCDIC FILE FOUND.  NOW CHECKING PARM FILE'
