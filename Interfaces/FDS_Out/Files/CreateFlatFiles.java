...
public class CreateFlatFiles {
	...
	private static final String SUMrs_query = "SELECT * FROM dbo.xx_imaps_inv_out_sum WHERE STATUS_FL = 'U' AND DIVISION = ? ORDER BY INVC_ID ";
	...
	private static final String DTLrs_NO_CSP_query = "SELECT "
		...
		+ "FROM dbo.xx_imaps_inv_out_dtl WHERE INVC_ID = ? AND SALES_TAX_AMT <> BILLED_AMT " //To exclude Tax Summary Detail Line From FDS
		+ "AND (ACCT_ID NOT IN (SELECT PARAMETER_VALUE FROM XX_PROCESSING_PARAMETERS WHERE PARAMETER_NAME = \'CSP_ACCT_ID\') " 
		+ "OR ACCT_ID IS NULL) " +
		"GROUP BY ri_billable_chg_cd, m_product_code, i_mach_type, tc_agrmnt, tc_prod_catgry, ts_dt, tc_tax, " +
		"bill_rt_amt, id, name, bill_lab_cat_cd, bill_lab_cat_desc, bill_fm_grp_no, " +
		"bill_fm_grp_lbl, rf_gsa_indicator, bill_fm_ln_no, bill_fm_ln_lbl, proj_abbrv_cd " +
	...
	private static final String HDR_INV_COUNT_query = "SELECT COUNT(1) as INVC_COUNT FROM dbo.xx_imaps_inv_out_sum WHERE STATUS_FL = 'U' AND DIVISION = ? ";
	
	private PreparedStatement PARAM_stmt;
	private static final String PARAM_query = "SELECT PARAMETER_VALUE FROM dbo.XX_PROCESSING_PARAMETERS WHERE INTERFACE_NAME_CD = 'FDS/CCS' AND PARAMETER_NAME = ? ";
	...
	public CreateFlatFiles(String propsPath) throws Exception {
		...
		CCSFlatFile = "IMAPS_TO_CCS_" + Integer.toString(month) + "_"
				+ Integer.toString(day) + "_" + Integer.toString(year) + ".txt";

		FDSFlatFile = "IMAPS_TO_FDS_" + Integer.toString(month) + "_"
				+ Integer.toString(day) + "_" + Integer.toString(year) + ".txt";

		TransactionReportFile = "Transaction_Report_" + Integer.toString(month)
				+ "_" + Integer.toString(day) + "_" + Integer.toString(year)
				+ ".txt";
		
		ErrorLogFile = "FDS_CCS_ERROR_LOG_" + Integer.toString(month)
				+ "_" + Integer.toString(day) + "_" + Integer.toString(year)
				+ ".txt";
...
//			Try to write Error to Log
	...
	public static void main(String[] args) throws Exception {
		...
		// input is dbname, username, and password
		CreateFlatFiles Interface = new CreateFlatFiles(args[0]); //, args[1], args[2], args[3]); // connection opened
