USE [IMAPSStg]
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[XX_R22_FIWLR_USDET_ARCHIVE]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[XX_R22_FIWLR_USDET_ARCHIVE]
GO
...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[XX_R22_FIWLR_USDET_ARCHIVE]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
 BEGIN
CREATE TABLE [dbo].[XX_R22_FIWLR_USDET_ARCHIVE](
	...
	[CREATION_DATE] [datetime] NOT NULL CONSTRAINT [NN_XX_R22_FIWLR_USDET_ARCH_CDATE]  DEFAULT (getdate()),
