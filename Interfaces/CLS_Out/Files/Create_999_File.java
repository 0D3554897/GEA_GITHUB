 ...
 *
 *
 *  CR8763 - Add Division 2G - 11-10-2016  george alvarez
 *         - also tried to add logging capability, it failed
 ******************************************************************************/


public class Create_999_File {

	//private static Logger     appLogger  = null;

	//MS SQL Server 2000 JDBC Driver Variables
	private java.sql.Connection con = null;
	private String databaseName = "A";
	private String userName = "B";
	private String password ="C";
	private String serverName = "D";
	private String STATUS_RECORD_NUM = "E"; 
	private String error_dir = "F";
	...
	//private static final String url = "jdbc:sqlserver://"; <-- no, this is normal jdbc
	private static final String portNumber = "1433";
	private static final String selectMethod = "cursor";
	
	// File Names and Locations Variables 
	private static final String Naked_CLS_999_File = "IMAPS_TO_CLS.BIN";
	private static final String Naked_CLS_ASCII_File = "IMAPS_TO_CLS_ASCII.TXT";
	private static final String Naked_ErrorLogFile = "IMAPS_TO_CLS_ERROR_LOG.TXT";
	private static final String Naked_ParameterCardFile = "F155PARM.TXT";
	private String Target_dir;

	private String CLS_999_File = "IMAPS_TO_CLS.BIN";
	private String CLS_ASCII_File = "IMAPS_TO_CLS_ASCII.TXT";
	private String ErrorLogFile = "IMAPS_TO_CLS_ERROR_LOG.TXT";
	private String ParameterCardFile = "F155PARM.TXT";
	...
	private static final String CLS_query = "SELECT * FROM dbo.XX_CLS_DOWN WHERE DIVISION = ? ";
	
	private ResultSet CLS_LOG_rs;
	private PreparedStatement CLS_LOG_stmt;
	private static final String CLS_LOG_query = "SELECT * FROM dbo.XX_CLS_DOWN_LOG " +
	...
	// Constructor
	  ...
	  private static String[] propKeys = {
		    "db.serverName","db.databaseName","db.userName","db.password","err.folder"
		  };
	...
	private Create_999_File(String propsPath, String Status_Num) throws Exception {
	    ...
	    error_dir = props.get("err.folder").toString();

	    ErrorLogFile = error_dir + Naked_ErrorLogFile;
	    
		STATUS_RECORD_NUM = Status_Num;		
		System.out.println("status record number is " + STATUS_RECORD_NUM);
		// Try to Create Error Log File Output
		try {
            System.out.println("Error Log About to be Created...");
		...
		try {
			ErrorLog.println("Attempting to Connect to the Database...");
                        System.out.println("Connect to db - try");
			con = this.getConnection();
		} 
		catch (Exception e) {
			// Try to write Error to Log
			System.out.print("cannot connect to db - e");
			try{
			...
			}catch (Exception etwo)
			{
				System.out.println("Error Connecting to Database and Creating Error Log");
                        System.out.println("cannot connect to db - e2");		
			}
		}
...
//			Try to write Error to Log
			...
			{
				System.out.println("Error Connecting to database and Creating Error Log");
                        System.out.println("Connection Unsuccessful e3");		
			}
		}
		return con;
	}// end getConnection
...
//			Try to write Error to Log
	...
	private void SetupCLSOutput()
	{
		...
		try {
			PARAM_rs = null;
			System.out.println("Get Output Folder First");

			PARAM_stmt.setString(1, "CLS");
			PARAM_stmt.setString(2, "FIL_DIR");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Target_dir = PARAM_rs.getString("PARAMETER_VALUE");

			System.out.println("Target Dir is " + Target_dir);
			System.out.println("Error folder (props file) is " + error_dir);
			System.out.println("These two locations must be exactly the same or error will occur in MOVE statement in SP");

			CLS_999_File = Target_dir + "IMAPS_TO_CLS.BIN";
			CLS_ASCII_File = Target_dir + "IMAPS_TO_CLS_ASCII.TXT";
			ParameterCardFile = Target_dir + "F155PARM.TXT";
		}catch (Exception e) 
		{
			System.out.println("Failed Query Target Dir; see error log ");
			//Try to write Error to Log
			ErrorLog.println("Error Loading CLS FIL_DIR Parameter From Database!!!");
			ErrorLog.println("Exception: " + e.getMessage());
			e.printStackTrace(ErrorLog);
		}
		...
		try {
            ...
            System.out.println("Creating files now: " + CLS_999_File + " and " + CLS_ASCII_File + " and " + ParameterCardFile);
			...
			ParameterCard = new java.io.PrintWriter(
					new java.io.OutputStreamWriter(
                        ...
                        System.out.println("Done creating files now");
...
//			Try to write Error to Log
			...
			{
				System.out.println("Really Bad! Error Creating CLS OutputStreams and Creating Error Log");		
			}
		}
	}
	...
	private void LoadConstantParameters(String DIVISION)
	{
		...
		try {
			...
			// set all parameters
			PARAM_stmt.setString(2, DIVISION+"_"+"COUNTRY_NUM");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Country_Number = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 3);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"LEDGER_CD");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Ledger_Code = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 2);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"FILE_ID");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			...
			PARAM_stmt.setString(2, DIVISION+"_"+"TOLI");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Type_Of_Ledger_Indicator = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 1);

			PARAM_stmt.setString(2, DIVISION+"_"+"DIVISION");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Division = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 2);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"LERU_NUM");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Ledger_Reporting_Unit = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 6);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"LEDGER_SOURCE_CD");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Ledger_Source = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 3);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"ACCOUNTANT_ID");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			...
			PARAM_stmt.setString(2, DIVISION+"_"+"USER_ID");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			User_ID = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 8);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"INPUT_TYPE_ID");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Input_Type_Identifier = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 1);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"FULFILLMENT_CHANNEL_CD");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
			Fulfillment_Channel = TSPC(PARAM_rs.getString("PARAMETER_VALUE"), 3);
			
			PARAM_stmt.setString(2, DIVISION+"_"+"CONFRMCD");
			PARAM_rs = PARAM_stmt.executeQuery();
			PARAM_rs.next();
	...
	private void LoadCLSrs(String Division)
	{
		//Select Invoice Summary Record Set
		try {
			CLS_rs = null;
			CLS_stmt.setString(1, Division);
	...
	private int Write_CLS_999_File() {
		...
		try {
			...
			if (con != null) {
				...
				System.out.println("Attempting to Write CLS 999 File...");


				
				// Constants for 16
				LoadConstantParameters("16");
				
				// Load CLS Transactions for 16
				LoadCLSrs("16");
				
				// For Each Transaction for 16
				while(CLS_rs.next())
				{
				...
				// Constants for 1M
				LoadConstantParameters("1M");
				
				// Load CLS Transactions for 1M
				LoadCLSrs("1M");
				...
				// Constants for 1P
				LoadConstantParameters("1P");
				
				// Load CLS Transactions for 1P
				LoadCLSrs("1P");
				
				// For Each Transaction for 1P
				while(CLS_rs.next())
				{
					Write_Current_Transaction();
				}	
				...
				//CR8763 - Add 2G
				// Constants for 2G
				LoadConstantParameters("2G");
				
				// Load CLS Transactions for 2G
				LoadCLSrs("2G");
				
				// For Each Transaction for 2G
				while(CLS_rs.next())
				{
					Write_Current_Transaction();
				}	
				...
				// Write 999 Byte Records to File
...
/*************** cannot get it to compile *****************
	  ...
	  private static void setupLogger() throws Exception {
	    ...
	    File   logfile = new File(logdir,"clsdown.txt");
	    ...
	    if (!rootLogger.getAllAppenders().hasMoreElements()) {
	      ...
	      appLogger = rootLogger.getLoggerRepository().getLogger("");
...
*************** cannot get it to compile *****************/
	...
	public static void main(String[] args) throws Exception {
               ...
               System.out.println("CLS_DOWN First Line Printed");
System.out.flush();
//ErrorLog.println("Error Log Created...just beginning");		
		...
		try {
		  setupLogger();
    		  ...
    		  e.printStackTrace();
                ...
                appLogger.debug("Executing Function: Logging Begins");
*************** cannot get it to compile *****************/

		Create_999_File Interface = new Create_999_File(args[0], args[1]);//, args[2], args[3], args[4]); // connection opened
