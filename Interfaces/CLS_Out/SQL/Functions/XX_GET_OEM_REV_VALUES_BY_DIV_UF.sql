...
/****** Object:  UserDefinedFunction [dbo].[XX_GET_OEM_REV_VALUES_BY_DIV_UF]    Script Date: 9/16/2022 11:14:10 AM ******/
...
CREATE FUNCTION [dbo].[XX_GET_OEM_REV_VALUES_BY_DIV_UF](@in_imaps_acct varchar(8), @in_div varchar(2), @in_which varchar(3))  
...
Usage: SELECT XX_GET_OEM_REV_VALUES_BY_DIV_UF(IMAPS_ACCT, DIV, WHICH VALUE YOU WANT)
...
IF @PARAM_CHECK IS NULL
	RETURN CAST('MISSING PARAMETER ERROR IN XX_GET_OEM_REV_VALUES_BY_DIV_UF - CHECK JOB LOG' AS INT)
...
WHERE Y.IMAPS_ACCT IN (SELECT IMAPS_ACCT FROM IMAPSSTG.DBO.XX_CLS_DOWN_HW_SW_MAP WHERE CLASS = 'REV')
