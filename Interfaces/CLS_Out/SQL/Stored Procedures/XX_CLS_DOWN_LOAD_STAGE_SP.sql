...
SET @DIVISION = '1M'
 ...
 IS NULL 
	...
	********/
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 21.	ARCHIVE YTD IMAGE'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
...
/************* THIS IS FOR DIVISION 16 ONLY *********************/
	...
	**************************************************************************/
	...
	-- D. REVERSE EXISTING REVENUE
	-- E. CREATE REVENUE ENTRY - HW
	-- F. CREATE REVENUE ENTRY - SW
	-- G. RESTORE DIVISION 16
	-- H. UPDATE CUSTOMER DATA FROM CMR
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 23(D) CREATE REVERSING ENTRY'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	SET @error_msg_placeholder1 = 'INSERT REVENUE REVERSALS INTO'
	SET @error_msg_placeholder2 = 'XX_CLS_DOWN_YTD'		

	INSERT INTO IMAPSSTG.DBO.XX_CLS_DOWN_THIS_MONTH_YTD
	SELECT Y.CLS_MAJOR, Y.CLS_MINOR, Y.CLS_SUB_MINOR, Y.IMAPS_ACCT, Y.IMAPS_PROJ_ID, 
			Y.L1_PROJ_SEG_ID, -1 * Y.DOLLAR_AMT, Y.GA_AMT, Y.OVERHEAD_AMT, Y.CONTRACT_NUM, 
			Y.IGS_PROJ, Y.SERVICE_OFFERING, Y.CUSTOMER_NUM, Y.MACHINE_TYPE_CD, Y.PRODUCT_ID, 
			'REVERSE ' + Y.DESCRIPTION2 , Y.BUSINESS_AREA, Y.MARKETING_AREA, Y.MARKETING_OFFICE, 
			Y.CONSOLIDATED_REV_BRANCH_OFFICE, Y.INDUSTRY, Y.ENTERPRISE_NUM_CD, Y.DIVISION
	FROM 
		IMAPSSTG.DBO.XX_CLS_DOWN_THIS_MONTH_YTD Y 
	WHERE Y.DIVISION IN (SELECT DISTINCT DIVISION FROM  IMAPSSTG.DBO.XX_CLS_DOWN_HW_SW_MAP)
			AND IMAPS_ACCT IN ('43-15-97','43-15-98')

	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 23(E) CREATE REVENUE ENTRY LINES FOR HW'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 23(F) CREATE REVENUE ENTRY LINES FOR SW'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 23(G) RESTORE DIVISION 16'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 23(H) UPDATE CMR INFO'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	SET @error_msg_placeholder1 = 'RESTORE XX_CLS_DOWN_YTD'
	SET @error_msg_placeholder2 = 'WITH DIVISION 16'	
...
/************* THIS IS FOR DIVISION 16 ONLY *********************/
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 24.	CALCULATE DIFFERENCE BETWEEN THIS MONTH AND LAST MONTH VALUES'
...
*/
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 25.	PERFORM PROJECT CODE DEFAULTING'
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 25.A'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	SET @error_msg_placeholder1 = 'UPDATE XX_CLS_DOWN'
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 25.B'
	SET @IMAPS_error_number = 204 -- Attempt to %1 %2 failed.
	SET @error_msg_placeholder1 = 'UPDATE XX_CLS_DOWN'
...
'CSI','BTO','WEB') THEN @IGSPROJ_FOR16
	...
	PRINT 'TIME:' + CAST(CAST(GETDATE() AS TIME) AS VARCHAR(5)) + ' ## STEP 24.C - SUCCESSFULLY REACHED END OF XX_CLS_DOWN_LOAD_STAGE_SP.SQL; NEXT - dbo.XX_CLS_DOWN_VALIDATE_CMR_SP'
