...
/****** Object:  StoredProcedure [dbo].[XX_CLS_DOWN_LOAD_STAGE_SP]    Script Date: 11/8/2022 9:54:08 AM ******/
...
*****/
	...
	SELECT 	COALESCE(IMAPSSTG.DBO.XX_BM_DIV_MMS_UF(IMAPSSTG.DBO.SB_GET_BUS_MEAS_DIV_CD_UF(imaps_proj_id) + '3','MAJ'),Y.CLS_MAJOR) AS MAJ,
			COALESCE(IMAPSSTG.DBO.XX_BM_DIV_MMS_UF(IMAPSSTG.DBO.SB_GET_BUS_MEAS_DIV_CD_UF(imaps_proj_id) + '3','MIN'),Y.CLS_MINOR) AS MNR,
			COALESCE(IMAPSSTG.DBO.XX_BM_DIV_MMS_UF(IMAPSSTG.DBO.SB_GET_BUS_MEAS_DIV_CD_UF(imaps_proj_id) + '3','SUB'),Y.CLS_SUB_MINOR) AS SUB,
			'', 
	...
	WHERE Y.DIVISION IN (SELECT DISTINCT DIVISION FROM  IMAPSSTG.DBO.XX_CLS_DOWN_HW_SW_MAP WHERE CLASS = 'REV')
			...
			AND Y.IGS_PROJ <> '144F'
			AND M.HWSW = 'HW'
			AND Y.L1_PROJ_SEG_ID NOT IN (SELECT IMAPS_LEVEL_1 FROM IMAPSSTG.DBO.XX_CLS_OEM_REV_EXCLUDES)
