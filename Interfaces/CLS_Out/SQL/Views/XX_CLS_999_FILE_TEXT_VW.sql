...
/****** Object:  View [dbo].[XX_CLS_999_FILE_TEXT_VW]    Script Date: 5/11/2022 6:50:37 PM ******/
...
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_LEDGER_CD')+SPACE(2),2) as Ledger_Code -- trailing to 2
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_FILE_ID')+SPACE(3),3) as THE_File_Id  -- trailing to 3
, '0000' as File_Sequence_No
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_TOLI')+SPACE(1),1) as Type_of_Ledger_IndicatoR -- trailing to 1
, DIVISION as Division
, left(coalesce(CLS_MAJOR,'') + SPACE(3),3) as Major  -- to 3 trailing spaces FIXED
, left(coalesce(CLS_MINOR,'') + SPACE(4),4) as Minor   -- to 4 trailing spaces FIXED
, left(coalesce(CLS_SUB_MINOR,'') + SPACE(4),4)  as Sub_Minor  -- to 4 trailing spaces FIXED
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_LERU_NUM') + SPACE(6),6) as Ledger_Reporting_Unit  -- trailing to 6
...
SPACE(2) AS BK  -- FILLER1  START = 59  LEN = 2
 ,left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_LEDGER_SOURCE_CD')+ SPACE(3),3) as Ledger_Source -- trailingto 3 FIX 20181016
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_ACCOUNTANT_ID')+ SPACE(3),3)  as Accountant_ID   -- trailingto 3 FIX 20181016
, left(Coalesce(VOUCHER_NUM,'') + SPACE(7),7) as Index_Number -- trailing spaces to 7
, SPACE(5)  as Pre_Index_Number  -- 5 blank spaces
...
, SPACE(3) as Accounting_Month_Fiscal    -- Filler(2)
, DOLLAR_AMT as AMT_LOCAL_CURRENCY
...
,SPACE(15) as Amount_US_Dollars
, left(isnull(MACHINE_TYPE_CD,'') + SPACE(4), 4) as Machine_Type  --TSPC(CLS_rs.getString("MACHINE_TYPE_CD"), 4)
, SPACE(3)  as Machine_Model --Filler(3)
, SPACE(12)  as Invoice_Number -- Filler(10)
, left('IMAPS ' + right( '0000' + coalesce(clslog.FY_SENT,''),4) + right( '00' + coalesce(clslog.MONTH_SENT,''),2) + SPACE(1) + coalesce(IMAPS_ACCT,'') + SPACE(30),30)  as Description_1 --TSPC( "IMAPS " + Accounting_Year + Accounting_Month_Local + " " + IMAPS_ACCT , 30) trailing  spaces to 30 FIXED
, left(coalesce(DESCRIPTION2,'') + SPACE(30), 30)  as Description_2 --TSPC(CLS_rs.getString("DESCRIPTION2"), 30) FIXED
...
SPACE(4) AS FMIN ,  --   START = 271  LEN = 4
 left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_TOLI') + SPACE(1), 1)  as Reference_Type_Of_Ledger_Ind --trailing to 1 FIXED
, division as Reference_Division
, left(coalesce(CLS_MAJOR,'') + SPACE(3),3)  as Reference_Major -- to 3 trailing spaces FIXED
, left(coalesce(CLS_MINOR,'') + SPACE(4),4)  as Reference_Minor -- to 4 trailing spaces  FIXED
, left(coalesce(CLS_SUB_MINOR,'') + SPACE(4),4)  as Reference_Sub_Minor --to 4 trailing spaces  FIXED
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_LERU_NUM') + SPACE(6),6) as Reference_LERU -- trailing to 6
,SPACE(6) AS FRV_DATE6 ,  --   START = 295  LEN = 6
SPACE(2) AS TAI ,  --   START = 301  LEN = 2
 left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_LEDGER_CD') + SPACE(2),2)  as HQ_Conversion_LC --trailing to2
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_INPUT_TYPE_ID') + SPACE(1),1)  as Input_Type_Identifier -- parameter INPUT_TYPE_ID trailing  to 1
...
SPACE(1) AS YTD_IND ,  --   START = 355  LEN = 1
 left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where parameter_name ='16_FULFILLMENT_CHANNEL_CD') + SPACE(3), 3) as  Fulfillment_Channel --parameter 16_FULFILLMENT_CHANNEL_CD trailing 3
,SPACE(7) AS PID ,  --   START = 359  LEN = 7
SPACE(2) AS REVAL_IND ,  -- FILLER7  START = 366  LEN = 2
  left(coalesce(BUSINESS_AREA,'') + SPACE(4),4) as Marketing_Division -- trailing 2 FIXED
,SPACE(2) AS SOC,  --   START = 372  LEN = 2
  left(coalesce(MACHINE_TYPE_CD, coalesce(PRODUCT_ID,'')) + SPACE(12),12)  as Part_Number  -- trailing 12 FIXED
,  SPACE(4) as  Exchange_Minor -- Filler(4)
,  SPACE(1)  as X_Org_Indicator -- Filler(1)
,  left(coalesce(MACHINE_TYPE_CD, coalesce(PRODUCT_ID,'')) + SPACE(12),12)  as Product_ID  -- trailing 12 FIXED
,  left(coalesce(CUSTOMER_NUM,'')+ SPACE(8),8) as Customer_Number  -- trailing 7 FIXED
,  SPACE(4)  as Feature_Number  -- Filler(4)
,  SPACE(2)  as Filler_1  -- Filler(1)
,  left(coalesce(MACHINE_TYPE_CD, coalesce(PRODUCT_ID,'')) + SPACE(12),12) as From_Product_ID --if MACHINE_TYPE_CD is blank then PRODUCT_ID else MACHINE_TYPE_CDto trailing 12 FIXED
,  SPACE(15)  as Quantity  -- Filler(15)
,  left(coalesce(MARKETING_AREA,'')+ SPACE(2),2) as Marketing_Area  -- trailing to 2 FIXED
...
COALESCE(LEFT(IMAPS_PROJ_ID + SPACE(7),7),SPACE(7)) AS CORP_USE7 ,  -- PACT NUMBER  START = 484  LEN = 7  MIMICS PROJECT FOR LINES
...
COALESCE(IMAPSSTG.DBO.XX_GET_CONTRACT_TYPE_CD_UF(IMAPS_PROJ_ID),SPACE(2)) AS CONTR_TYPE ,  --   START = 524  LEN = 2
...
SPACE(9) AS SERIAL_NBR ,  --   START = 555  LEN = 9
  left(isnull(IGS_PROJ,'') + SPACE(7),7) as IGS_Project_No  --trailing to 7
,SPACE(12) AS DLVY_NOTE_NBR ,  --   START = 571  LEN = 12
SPACE(6) AS ORDER_NBR ,  -- FILLER10  START = 583  LEN = 6
  left(isnull(CONTRACT_NUM,'') + SPACE(15),15) as Contract_Number  -- trailing to 15
,  left(coalesce(MACHINE_TYPE_CD, coalesce(PRODUCT_ID,'')) + SPACE(12),12) as Service_Product_ID  -- FIXED
,  left(coalesce(MACHINE_TYPE_CD, coalesce(PRODUCT_ID,'')) + SPACE(12),12) as OEM_Product_ID -- FIXED
,SPACE(5) AS ISIC_CODE ,  --   START = 628  LEN = 5
SPACE(9) AS AGREE_REF_NBR ,  --START = 633  LEN = 9
  left(isnull(MARKETING_OFFICE,'') + SPACE(3),3) as Marketing_Branch_Office   --trailing to 3
,SPACE(3) AS UNIT_BIL ,  --   START = 645  LEN = 3
SPACE(3) AS UNIT_USER ,  --   START = 648  LEN = 3
SPACE(8) AS CUST_NBR_USER,  --   START = 651  LEN = 8
left(CUSTOMER_NUM + SPACE(8),8) AS CUST_NBR_BIL ,  --   START = 659  LEN = 8
SPACE(8) AS CUST_NBR_OWNER ,  --   START = 667  LEN = 8
SPACE(8) AS CUST_NBR_PAY ,  --   START = 675  LEN = 8
SPACE(7) as INV_NBR,  --   START = 683  LEN = 7
...
left(isnull(CONSOLIDATED_REV_BRANCH_OFFICE,'') + SPACE(3),3) as Consolidated_Revenue_BO  -- trailing to 3
