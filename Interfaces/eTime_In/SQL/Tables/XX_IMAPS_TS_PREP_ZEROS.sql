/****** Object:  Table [dbo].[XX_IMAPS_TS_PREP_ZEROS]    Script Date: 08/16/2006 1:55:30 PM ******/
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[XX_IMAPS_TS_PREP_ZEROS]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[XX_IMAPS_TS_PREP_ZEROS]
GO

if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[XX_IMAPS_TS_PREP_ZEROS]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
 BEGIN
CREATE TABLE [dbo].[XX_IMAPS_TS_PREP_ZEROS] (
