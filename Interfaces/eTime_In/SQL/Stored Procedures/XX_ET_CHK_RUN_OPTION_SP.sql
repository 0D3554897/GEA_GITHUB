IF OBJECT_ID('dbo.XX_ET_CHK_RUN_OPTION_SP') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.XX_ET_CHK_RUN_OPTION_SP
    IF OBJECT_ID('dbo.XX_ET_CHK_RUN_OPTION_SP') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.XX_ET_CHK_RUN_OPTION_SP >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.XX_ET_CHK_RUN_OPTION_SP >>>'
END
go
SET ANSI_NULLS ON
go
...
CREATE PROCEDURE dbo.XX_ET_CHK_RUN_OPTION_SP
(
...
Modified for CR-3617 03/16/11 Shared ID Issue
...
-- set local constants
SET @SP_NAME = 'XX_ET_CHK_RUN_OPTION_SP'
...
SET @login_name = '' --Modified for CR-3617
...
SET @cmd = 'bcp' + @SPACE
SET @cmd = @cmd + @DOUBLE_QUOTE + @in_IMAPS_db_name + @PERIOD + @in_IMAPS_table_owner + @PERIOD + 'XX_IMAPS_ET_IN_UTIL' + @DOUBLE_QUOTE + @SPACE
...
SET @cmd = @cmd + '-P' + @SPACE + @DOUBLE_QUOTE + @in_user_password + @DOUBLE_QUOTE*/ --Modified for CR-3617 03/16/11

PRINT 'Populate utility table XX_IMAPS_ET_IN_UTIL with timesheet data using bcp ...'
PRINT @cmd

-- guarantee that the target table is empty
TRUNCATE TABLE dbo.XX_IMAPS_ET_IN_UTIL
...
go
IF OBJECT_ID('dbo.XX_ET_CHK_RUN_OPTION_SP') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.XX_ET_CHK_RUN_OPTION_SP >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.XX_ET_CHK_RUN_OPTION_SP >>>'
go
