IF OBJECT_ID('dbo.XX_ET_VALIDATE_SOURCE_DATA_SP') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.XX_ET_VALIDATE_SOURCE_DATA_SP
    IF OBJECT_ID('dbo.XX_ET_VALIDATE_SOURCE_DATA_SP') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.XX_ET_VALIDATE_SOURCE_DATA_SP >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.XX_ET_VALIDATE_SOURCE_DATA_SP >>>'
END
go
SET ANSI_NULLS ON
go
...
CREATE PROCEDURE dbo.XX_ET_VALIDATE_SOURCE_DATA_SP
(
...
/******************************************************************************************************
Name:       XX_ET_VALIDATE_SOURCE_DATA_SP
Author:     HVT
Created:    09/19/2005
Purpose:    After data from the source input file have been loaded into staging tables, inspect
            ...
            Called by XX_LOAD_ET_STAGING_DATA_SP.
Parameters: 
Result Set: None
Notes:
...
CR-4886     Modified for CR-4886, Actuals implementation
*******************************************************************************************************/
...
-- set local constants
SET @SP_NAME = 'XX_ET_VALIDATE_SOURCE_DATA_SP'
SET @SPACE_CHAR = ' '
...
SELECT @rowcount = COUNT(1) FROM dbo.XX_IMAPS_ET_IN_TMP

IF @rowcount = 0
   BEGIN
      EXEC dbo.XX_ERROR_MSG_DETAIL
         @in_error_code          = 305,
         @in_placeholder_value1  = 'eTime interface',
         @in_display_requested   = 0,
         @in_calling_object_name = @SP_NAME,
         @out_msg_text           = @error_msg OUTPUT

      SET @error_desc = 'No data in the staging table XX_IMAPS_ET_IN_TMP.'
...
SELECT @rowcount = COUNT(1) FROM dbo.XX_IMAPS_ET_FTR_IN_TMP
...
IF @rowcount = 0
   BEGIN
    ...
    @in_display_requested   = 0,
      ...
      SET @error_desc = 'No data in the staging table XX_IMAPS_ET_FTR_IN_TMP.'
      SET @out_STATUS_DESCRIPTION = @error_msg + @SPACE_CHAR + @error_desc
      SET @error_msg = @error_msg + CHAR(13) + @error_desc + ' [' + @SP_NAME + ']'
      PRINT @error_msg
      RETURN(1)
...
WHILE (@@FETCH_STATUS = 0)
   BEGIN
      ...
      IF (ISNUMERIC(@in_TS_YEAR) = 1 AND ISNUMERIC(@in_TS_MONTH) = 1 AND ISNUMERIC(@in_TS_DAY) = 1) AND
         ...
         BEGIN
            ...
            IF CAST(@in_TS_MONTH AS NUMERIC) > 12
               BEGIN
                  SET @bad_data_flag = 1
                  SET @error_desc = 'Month part value (XX_IMAPS_ET_IN_TMP.TS_MONTH) is greater than 12.'
                  GOTO bl_BAD_DATA_EXIT -- jump to the branch label
               END
            IF CAST(@in_TS_DAY AS NUMERIC) > 31
               BEGIN
                  SET @bad_data_flag = 1
                  SET @error_desc = 'Day part value (XX_IMAPS_ET_IN_TMP.TS_DAY) is greater than 31.'
      ...
      IF NOT @in_RECORD_TYPE IN ('R', 'C','N','D') -- R=Regular, C=Correcting, N=Reverse, D=positive --Added CR-4886
         BEGIN
            SET @bad_data_flag = 1
...
SELECT @c_tot_ftr_reg_time = TOTAL_REG_TIME, @c_tot_ftr_rec_count = TOTAL_RECORDS
  FROM dbo.XX_IMAPS_ET_FTR_IN_TMP
...
go
IF OBJECT_ID('dbo.XX_ET_VALIDATE_SOURCE_DATA_SP') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.XX_ET_VALIDATE_SOURCE_DATA_SP >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.XX_ET_VALIDATE_SOURCE_DATA_SP >>>'
go
GRANT EXECUTE ON dbo.XX_ET_VALIDATE_SOURCE_DATA_SP TO public
go
