IF OBJECT_ID('dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP
    IF OBJECT_ID('dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP >>>'
END
go
SET ANSI_NULLS ON
go
...
CREATE PROCEDURE [dbo].[XX_R22_ETIME_SIMULATE_COSTPOINT_SP] 
(
...
Version: 	2.0 	Modified for CR-1543 COMPANY_ID added tp 05/01/2008
                    ...
                    Modified for CR-1649 11/05/08 changes
************************************************************************************************/  
BEGIN
	...
	LAB_CST_AMT = NULL
	-- For R22 labor cost will be left Blank
	FROM XX_R22_IMAPS_TS_PREP_TEMP TS
	INNER JOIN
	IMAR.DELTEK.LAB_ACCT_GRP_DFLT LAGD
	ON
	( 
	 ...
	 AND
	 LAGD.ACCT_GRP_CD = (SELECT ACCT_GRP_CD FROM IMAR.DELTEK.PROJ WHERE PROJ_ABBRV_CD = TS.PROJ_ABBRV_CD AND COMPANY_ID=@in_COMPANY_ID) -- Modified for CR-1543
	)
	*/
	...
	( 
	 ...
	 AND
	 PTAM.ACCT_GRP_CD = (SELECT ACCT_GRP_CD FROM IMAR.DELTEK.PROJ WHERE PROJ_ABBRV_CD = TS.PROJ_ABBRV_CD AND COMPANY_ID=@in_COMPANY_ID) -- Modified for CR-1543
	)
	...
	ON
	(PTD.PROJ_ID = (SELECT PROJ_ID FROM IMAR.DELTEK.PROJ WHERE PROJ_ABBRV_CD = TS.PROJ_ABBRV_CD AND COMPANY_ID=@in_COMPANY_ID) -- Modified for CR-1543
	)



/* Not needed for Div.22
...
go
IF OBJECT_ID('dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.XX_R22_ETIME_SIMULATE_COSTPOINT_SP >>>'
go
