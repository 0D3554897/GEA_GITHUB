IF OBJECT_ID('dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP
    IF OBJECT_ID('dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP >>>'
END
go
SET ANSI_NULLS ON
go
...
go
CREATE procedure [dbo].[XX_R22_INSERT_TS_RESPROC_MISCODE_SP]
as

/************************************************************************************************  
Name:       	dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP
Author:     	Tejas Patel
Created:    	07/2008  
Purpose:    	
	...
	This process reclass labor records of newly mapped employees and also reprocesses previously miscoded records.
	The Reclass is required if employee standard hours have changed since miscoded and if they were not reclassed earlier.
	IF the labor records were reclassed then they will not be consider for the process.

execute dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP
...
Version: 	1.0
					...
					Modified for Rounding issue, Split Week BO line issue, Split Week miscode issue 11/11/08	
                    CR-1821     Modified for Account Reclass Map 		
                    CR-1821     Modified to include Friday function in Delete statement
					CR-1901/DR-1875		Modified for Missing TC process as Regular
                                        Modified Correcting refdate logic
                                        Last Modi 3-10-2009
					CR-1921/CR-1926  Modified for Split BAckout lines issues
                    CR-2042         Modified for Backout line day multiplier logic 
                    DR-2230         Modified for backout line issue 
                    DR-2809         Modified for ts_hdr_seq conversion issue
************************************************************************************************/ 
BEGIN
DECLARE
        @in_EMPL_ID            char(12),
...
-- Delete Backing out lines from PREP_TEMP where the total TC hours=Std Hours
-- We will load this backing out lines again
-- This is allow us to recalculate the backing out line based on their current standard hours in ELI
-- This will also stop us from getting duplicate backing out lines
-- Friday function added so in case if there is a split week and this will combine them in one group
DELETE from dbo.XX_R22_IMAPS_TS_PREP_TEMP
FROM dbo.XX_R22_IMAPS_TS_PREP_TEMP ts
WHERE notes like '%-C-%'
and empl_id+ts_dt not in (select empl_id+ts_dt from 	xx_r22_imaps_ts_prep_temp 
							where notes like '%-A-%' or notes  like '%-B-%')
-- This is to ignore only -C- type records that was miscoded earlier
-- And their parent records got processed (-A- and -B-)
and empl_id+convert(char(10),dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(TS_DT),101)   		-- Friday Function added CR-1821/01/15/09
       IN (SELECT empl_id+convert(char(10),dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(TS_DT),101) 
			FROM  dbo.XX_R22_IMAPS_TS_PREP_TEMP t2 
				where substring(t2.notes, 1, charindex('-', t2.notes, CHARINDEX('-', t2.notes, 1)+1)-1) =
								substring(ts.notes, 1, charindex('-', ts.notes, CHARINDEX('-', ts.notes, 1)+1)-1)
				and empl_id=ts.empl_id and dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(ts_dt)=dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(ts.ts_dt)
				and s_ts_type_cd<>'C')

--Req# 3.4.6
--Exempt Employee Split
   ...
   SELECT  ts.empl_id, empl_lab.effect_dt, empl_lab.end_dt, empl_lab.exmpt_fl,
    ...
    (select sum(cast( chg_hrs as numeric (14,2))) 
        ...
        where empl_id=ts.EMPL_ID
			...
			and isnull(dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(correcting_ref_dt),'01-01-2078')=isnull(dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF

(ts.correcting_ref_dt),'01-01-2078')
     ...
     convert(numeric(14,2),sum(CONVERT(numeric(14,2),ts.chg_hrs))* (empl_lab.std_est_hrs/52)/
                            (select abs(sum(cast( chg_hrs as numeric(14,2))))
                                ...
                                where empl_id=ts.EMPL_ID
                                       ...
                                       and dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(TS_DT)=dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(ts.TS_DT)
									-- Added 03/08/2009 This will calculate sum of hrs for N, D lines
									and isnull(dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(correcting_ref_dt),'01-01-2078')=isnull

(dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(ts.correcting_ref_dt),'01-01-2078')
...
where exmpt_fl='Y'
    -- Miscode specific statements. This exclude the employees that was calculated last time
    --AND ts.notes not like '%-A-%' and ts.notes not like '%-B-%' and ts.notes not like '%-C-%'
	-- Added 11/05/2008
	AND ts.empl_id+ts_dt not in (select empl_id+ts_dt from 	xx_r22_imaps_ts_prep_temp where notes like '%-C-%')

--and ts.empl_id<='R00008'
    ...
    */
    ...
    SELECT TS_DT      ,EMPL_ID      ,S_TS_TYPE_CD      ,WORK_STATE_CD      ,FY_CD      ,PD_NO  ,SUB_PD_NO
        ...
        --,convert(decimal(14,2),(convert(decimal(14,2),chg_hrs)*@in_hrs_for_def_acc_factor)) CHG_HRS
          ,CASE WHEN abs(@in_STD_EST_HRS)>=abs(@in_total_by_tc) THEN convert(decimal(14,2),chg_hrs)  -- Modified added 11/11/2008 -- ABS Added CR-1901
          ...
          ,EFFECT_BILL_DT      ,PROJ_ACCT_ABBRV_CD ,
		    ...
		    +CASE WHEN abs(@in_STD_EST_HRS)=abs(@in_total_by_tc) THEN '-'				-- Modified 08/28/2008
			    ELSE '-A-'
			    END
    ...
    where rtrim(ts.empl_id)=rtrim(@in_empl_id)
       ...
       and ts.bill_lab_cat_cd=@in_bill_lab_cat_cd
        ...
        -- Miscode specific statements. This exclude the employees that was calculated last time
        AND ts.notes not like '%-A-%' and ts.notes not like '%-B-%' and ts.notes not like '%-C-%'
    --PRINT @in_hrs_for_def_acc_factor

    -- 2nd Insert with Special Project Account
    ...
    SELECT TS_DT      ,EMPL_ID      ,S_TS_TYPE_CD      ,WORK_STATE_CD      ,FY_CD      ,PD_NO      ,SUB_PD_NO
          ...
          ,CASE WHEN abs(@in_STD_EST_HRS)>=abs(@in_total_by_tc) THEN 0.00 -- Modified added 11/11/2008 -- Added CR-1901 ABS for N lines
...
-- Delete the records created with zero hours this happens when the total timecards hours are same as standard hours
...
DECLARE cursor_three CURSOR FAST_FORWARD FOR
 ...
 and rtrim(org_id)=rtrim(empl_lab.org_id) 
  ...
  and proj_abbrv_cd is not null 
	...
	/* Modified on 11/05/08 - Changed Reclass Logic
	(SELECT distinct ACCT_ID
	from XX_R22_ACCT_RECLASS map
	where map.lab_grp_type=empl_lab.lab_grp_type
			and map.proj_id='RRDE' 
			and map.acct_grp_cd ='RRD') reclass_acct_id,*/
	ISNULL((SELECT distinct map.acct_id
			from XX_R22_ACCT_RECLASS map
			where map.lab_grp_type=empl_lab.lab_grp_type
    ...
    -- Miscode specific statements. This exclude the employees that was calculated last time
    --AND ts.notes not like '%-A-%' and ts.notes not like '%-B-%' and ts.notes not like '%-C-%'
    -- Modified 11/05/2008 R is with A and B, C is with -C-
    --and ts.empl_id+ts.TS_DT not in (select empl_id+TS_DT from dbo.XX_R22_IMAPS_TS_PREP_TEMP where notes like '%-C-%')
    and ts.empl_id+ts.TS_DT not in (select empl_id+TS_DT from dbo.XX_R22_IMAPS_TS_PREP_TEMP where notes like '%-A-%' OR notes like '%-B-%')
...
BEGIN
	-- Added 11/11/2008 Logic to create BO line with standard hours if TS is a split and wasn't reclassed earlier.
	IF NOT EXISTS (
					select  count(1)
					from dbo.XX_R22_IMAPS_TS_PREP_TEMP ts
					where empl_id =@in_empl_id and TS_DT=@in_ts_dt -- Friday function removed CR-1921
					and empl_id+ convert(char(10),TS_DT,101)+s_ts_type_cd 
    					in (select empl_id+convert(char(10),(TS_DT),101)+s_ts_type_cd 
        					from dbo.XX_R22_IMAPS_TS_PREP_config_errors where status_record_num_reprocessed is not null)
        			)
...
select 
@out_notes=cast(max(cast (substring(notes, CHARINDEX('-', notes, 1)+1, (CHARINDEX('-', notes, (CHARINDEX('-', notes, 1)+1))-CHARINDEX('-', notes, 1)-1) ) as numeric)) as 

varchar)
 from dbo.XX_R22_IMAPS_TS_PREP_TEMP a
where empl_id=@in_empl_id
and s_ts_type_cd='R'
and TS_DT=@in_TS_DT -- CR-1921 Friday Function is removed
...
INSERT INTO dbo.XX_R22_IMAPS_TS_PREP_TEMP2
      ...
      ,ORG_ABBRV_CD           ,PROJ_ABBRV_CD           ,TS_HDR_SEQ_NO           ,EFFECT_BILL_DT           ,PROJ_ACCT_ABBRV_CD
 ,NOTES)
SELECT DISTINCT TS_DT      ,EMPL_ID      ,'C' S_TS_TYPE_CD      ,WORK_STATE_CD      ,FY_CD      ,@in_PD_NO      ,@in_SUB_PD_NO
 ,TS_DT CORRECTING_REF_DT      ,'BO' PAY_TYPE       ,GENL_LAB_CAT_CD      ,S_TS_LN_TYPE_CD  ,NULL LAB_CST_AMT
      ...
      (select cast(max(cast(ts_hdr_seq_no as decimal(4,0)))+1 as varchar(3)) --Modified DR-2809 09/27/2010
	    ...
	    and s_ts_type_cd in ('R', 'N','C','D') -- Added Tejas 01/27/08
			...
			ELSE convert(char(10),ts_dt,120) -- Modified Friday Function removed CR-1921
...
DECLARE cursor_four CURSOR FAST_FORWARD FOR
   ...
   and line_type='BACKOUT'),'99-99-99') ACCT_ID,
	...
	(ts.empl_id = empl_lab.empl_id
		and 
		(
			(ts.S_TS_TYPE_CD = 'R' AND cast(dbo.XX_GET_FRIDAY_FOR_TS_WEEK_DAY_UF(ts.TS_DT) as datetime) BETWEEN empl_lab.EFFECT_DT AND empl_lab.END_DT)
			OR
			(ts.S_TS_TYPE_CD in ('C','N','D') AND cast(ts.CORRECTING_REF_DT as datetime) BETWEEN empl_lab.EFFECT_DT AND empl_lab.END_DT)
		)
	)
...
BEGIN
	...
	IF NOT EXISTS (
					select  count(1)
					from dbo.XX_R22_IMAPS_TS_PREP_TEMP ts
					where empl_id =@in_empl_id and (TS_DT)=@in_ts_dt -- Friday Func removed CR-1921
					and empl_id+ convert(char(10),(TS_DT),101)+s_ts_type_cd  -- Friday Func removed CR-1921
    					in (select empl_id+convert(char(10),(TS_DT),101)+s_ts_type_cd 
        					from dbo.XX_R22_IMAPS_TS_PREP_config_errors where status_record_num_reprocessed is not null)
        			)
...
*/
    ...
    select 
    @out_notes=cast(max(cast (substring(notes, CHARINDEX('-', notes, 1)+1, (CHARINDEX('-', notes, (CHARINDEX('-', notes, 1)+1))-CHARINDEX('-', notes, 1)-1) ) as numeric)) 

as varchar)
     from dbo.XX_R22_IMAPS_TS_PREP_TEMP a
    where empl_id=@in_empl_id
    and s_ts_type_cd='R'
	and TS_DT=@in_TS_DT -- Removed friday func CR-1921
    -- Miscode specific statements. This exclude the employees that was calculated last time
    AND notes not like '%-A-%' and notes not like '%-B-%' and notes not like '%-C-%'
...
SELECT DISTINCT TS_DT      ,EMPL_ID      ,'C' S_TS_TYPE_CD      ,WORK_STATE_CD      ,FY_CD      ,@in_PD_NO      ,@in_SUB_PD_NO
      ,TS_DT CORRECTING_REF_DT      ,'BO' PAY_TYPE   ,GENL_LAB_CAT_CD      ,S_TS_LN_TYPE_CD      ,NULL LAB_CST_AMT
      ,-(@in_STD_EST_HRS) CHG_HRS
      ,WORK_COMP_CD      ,LAB_LOC_CD      ,@in_empl_org_id      ,@in_reclass_acct_id      ,PROJ_ID      ,@out_BILL_LAB_CAT_CD
      ,REF_STRUC_1_ID      ,REF_STRUC_2_ID      ,ORG_ABBRV_CD      ,@in_reclass_proj_abbrv_cd      ,
      (select cast(max(cast(ts_hdr_seq_no as decimal(4,0)))+1 as varchar(3)) --Modified DR-2809 09/27/2010
...
BEGIN
	...
	--BEGIN Update Miscode Table 
	-- Added 11/05/2008
	-- Delete non-processed miscode table records that is now exists in the TEMP2 table
	-- TEMP2 records are the one that just had mapped employee id
	DELETE FROM dbo.XX_R22_IMAPS_TS_PREP_CONFIG_ERRORS 
	-- select * from dbo.XX_R22_IMAPS_TS_PREP_CONFIG_ERRORS
	WHERE STATUS_RECORD_NUM_REPROCESSED IS NULL
	AND substring(notes, 1, charindex('-', notes, CHARINDEX('-', notes, 1)+1)-1)
    	IN (SELECT substring(t2.notes, 1, charindex('-', t2.notes, CHARINDEX('-', t2.notes, 1)+1)-1) FROM  dbo.XX_R22_IMAPS_TS_PREP_TEMP2 t2)
	
	-- Insert Records processed based on new employee id to miscode table
	INSERT INTO dbo.XX_R22_IMAPS_TS_PREP_CONFIG_ERRORS
	(STATUS_RECORD_NUM_CREATED, TS_DT, EMPL_ID, S_TS_TYPE_CD, WORK_STATE_CD, FY_CD, PD_NO, ACCT_ID,
	...
	SELECT 
	substring(notes,1, CHARINDEX('-', notes, 1)-1) STATUS_RECORD_NUM,
	TS_DT, EMPL_ID, S_TS_TYPE_CD, WORK_STATE_CD, FY_CD, PD_NO, ACCT_ID,
	SUB_PD_NO, CORRECTING_REF_DT, PAY_TYPE, LAB_CST_AMT, CHG_HRS,
	LAB_LOC_CD, PROJ_ID, BILL_LAB_CAT_CD, PROJ_ABBRV_CD, TS_HDR_SEQ_NO,
	EFFECT_BILL_DT, NOTES
	FROM dbo.XX_R22_IMAPS_TS_PREP_TEMP2 t
	WHERE 
   	substring(t.notes, 1, charindex('-', t.notes, CHARINDEX('-', t.notes, 1)+1)-1)
   	NOT IN (SELECT substring(t2.notes, 1, charindex('-', t2.notes, CHARINDEX('-', t2.notes, 1)+1)-1) 
            FROM  dbo.XX_R22_IMAPS_TS_PREP_CONFIG_ERRORS t2 where pay_type<>'BO'
            )
    -- Added pay_type<>'BO'   	This is to avoid situation where backout line was processed
    -- CR-1901 03/10/2009
    -- We uses max notes value for BO line.
	-- END Update Miscode Table


	-- Delete records from TS_PRE_TEMP for Exempt employees where labor records were not processed earlier.
	-- We will delete them before loading records from TEMP2 table.
	...
	where exmpt_fl='Y'
    AND notes not like '%-A-%' and notes not like '%-B-%' and notes not like '%-C-%'
    -- start added on 03/10/2009 CR-1901
    AND substring(notes, 1, charindex('-', notes, CHARINDEX('-', notes, 1)+1)-1)
          IN (SELECT substring(t2.notes, 1, charindex('-', t2.notes, CHARINDEX('-', t2.notes, 1)+1)-1) FROM  dbo.XX_R22_IMAPS_TS_PREP_TEMP2 t2)
    -- end add on 03/10/09
    -- commented out on 03/10/2009
...
-- Delete any backout rows created with the current run when the same backout line was processed earlier in config error table
-- CR-1901 03/09/2009
delete from xx_r22_imaps_ts_prep_temp
where notes in (select notes from xx_r22_imaps_ts_prep_config_errors 
                where status_record_num_reprocessed is not null and notes like '%-C-%' )

-- Also delete the same above record if they are processed earlier but backout line created again by miscode process.
delete from xx_r22_imaps_ts_prep_config_errors
where notes in (select notes from xx_r22_imaps_ts_prep_config_errors 
                where status_record_num_reprocessed is not null and notes like '%-C-%' )
and notes like '%-C-%'
...
--DELETE from IMAPSStg.dbo.XX_R22_IMAPS_TS_PREP_TEMP2
	...
	PRINT 'ERROR UPDATING TABLE TEMP2'
	RETURN(1)
...
go
IF OBJECT_ID('dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.XX_R22_INSERT_TS_RESPROC_MISCODE_SP >>>'
go
