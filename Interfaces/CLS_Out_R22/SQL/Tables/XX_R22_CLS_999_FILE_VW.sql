...
/****** Object:  View [dbo].[XX_R22_CLS_999_FILE_VW]    Script Date: 9/9/2020 2:54:21 PM ******/
DROP VIEW [dbo].[XX_R22_CLS_999_FILE_VW]
GO
...
usage: select * from imapsstg.dbo.XX_R22_CLS_999_FILE_VW
...
CREATE VIEW [dbo].[XX_R22_CLS_999_FILE_VW]
AS

Select 
-- reconcile field lengths to the spec, page 1: 3,2,3,4,1
(select LEFT(parameter_value,3) from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='COUNTRY_NUM')as Country_Number -- trailing to 3
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='LEDGER_CD')+Filler_2,2) as Ledger_Code -- trailing to 2
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='FILE_ID')+ Filler_3,3) as File_Id  -- trailing to 3
, '0000' as File_Sequence_No
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='TOLI') + Filler_1,1) as Type_of_Ledger_Indicator -- trailing to 1
-- reconcile field lengths to the spec, page 2: 2,3,4,4,6

/***
,CASE left(CLS_MAJOR,1)
  WHEN '0' THEN left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='BAL_SHT_DIVISION') + Filler_2,2) --trailing to 2
  WHEN '1' THEN left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='BAL_SHT_DIVISION') + Filler_2,2) --trailing to 2
  WHEN '2' THEN left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='BAL_SHT_DIVISION') + Filler_2,2) --trailing to 2
  ELSE left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='DIVISION') + Filler_2,2) --trailing to 2
end 
***/
,coalesce(division,'XX') AS Division
-- next line is superseded by above
--, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='DIVISION') + Filler_2,2) as Division --trailing to 2
...
,LEFT(LERU_NUM + Filler_6,6) AS Ledger_Reporting_Unit  -- trailing to 6
...
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='LEDGER_SOURCE_CD')+ Filler_3,3) as Ledger_Source -- trailing to 3 FIX 20181016
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='ACCOUNTANT_ID')+ Filler_3,3)  as Accountant_ID   -- trailing to 3 FIX 20181016
...
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='TOLI') + Filler_1, 1)  as Reference_Type_Of_Ledger_Ind --trailing to 1 FIXED
/****
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='DIVISION') + Filler_2, 2) as Reference_Division  --trailing to 2 FIXED
****/
,coalesce(division,'XX') as Reference_Division
...
,LEFT(LERU_NUM + Filler_6,6)  as Reference_LERU -- trailing to 6
--, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='LERU_NUM') + Filler_6, 6)  as Reference_LERU -- trailing to 6
, Filler_6  as File_Record_Verification_Run_Date  --Filler(6)
--50th field

, Filler_2 as Transfer_Account_Indicator  --Filler(2)
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='LEDGER_CD') + Filler_2,2)  as HQ_Conversion_LC --trailing to 2
/*****
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='DIVISION') + Filler_2,2)  as HQ_Conversion_Division --trailing to 2
*****/
,coalesce(division,'XX') as HQ_Conversion_Division
...
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='LERU_NUM')+Filler_6,6)  as HQ_Conversion_LERU -- trailing to 6
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='INPUT_TYPE_ID') + Filler_1,1)  as Input_Type_Identifier -- parameter INPUT_TYPE_ID trailing  to 1
...
, left ((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='FILE_ID') + Filler_3,3)  as Reference_File_ID  -- trailing to 3
...
, left((select parameter_value from IMAPSSTG.dbo.XX_PROCESSING_PARAMETERS where INTERFACE_NAME_CD = 'CLS_R22' AND  parameter_name ='FULFILLMENT_CHANNEL_CD') + Filler_3, 3) 
...
,  Filler_2  asRevaluation_Indicator -- Filler(2)
...
(select top 1 * from IMAPSSTG.dbo.XX_R22_CLS_DOWN_LOG order by status_record_num desc) clslog,
(
