...
FOR %%V IN (%1) DO FOR /F "tokens=1-5 delims=/: " %%J IN ("%%~tV") DO IF EXIST ..\csv2db\ora_log\o16_%%~nV%%L%%J%%K_%%M%%N%%~xV (ECHO Cannot rename %%V) ELSE (COPY /Y "%%V" o16_%%~nV%%L%%J%%K_%%M%%N%%~xV)
