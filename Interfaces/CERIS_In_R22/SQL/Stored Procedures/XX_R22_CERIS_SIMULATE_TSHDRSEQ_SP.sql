IF OBJECT_ID('dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP
    IF OBJECT_ID('dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP >>>'
END
go
SET ANSI_NULLS ON
go
...
CREATE PROCEDURE [dbo].[XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP] 
(
...
Purpose:  This procedure will update ts_hdr_seq_no for all N/D timesheets.  	
	...
	THIS ENTIRE STORED PROCEDURE IS FOR CR-2350
...
DECLARE EMPL_ID_CURSOR CURSOR FAST_FORWARD FOR
    SELECT DISTINCT EMPL_ID, convert(datetime,TS_DT) as TS_DT, convert(datetime,CORRECTING_REF_DT) as CORRECTING_REF_DT 
	    FROM DBO.XX_R22_CERIS_RETRO_TS_PREP where s_ts_type_cd not in ('R')
...
go
IF OBJECT_ID('dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.XX_R22_CERIS_SIMULATE_TSHDRSEQ_SP >>>'
go
