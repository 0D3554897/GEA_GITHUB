...
/****** Object:  StoredProcedure [dbo].[XX_SEND_STATUS_MAIL_SP]    Script Date: 6/4/2020 12:55:06 PM ******/
DROP PROCEDURE [dbo].[XX_SEND_STATUS_MAIL_SP]
GO
...
CREATE PROCEDURE [dbo].[XX_SEND_STATUS_MAIL_SP] 
( 
@in_StatusRecordNum  integer,
@in_Attachments      varchar(300) = NULL,  -- DR-12027
@out_SystemErrorCode integer = NULL OUTPUT 
)
AS
/**************************************************************************************************************
...
CR11604     12/11/2019 HVT - Correct parameter @USR_APP_CODE's value for the first XX_GET_LOOKUP_DATA call.

DR12027     05/07/2020 HVT - Fix the problem of @in_Attachments, defined as CHAR(300), that causes space
            characters to be padded to its RHS and results in Error 14628 issued by system procedure
            sp_GetAttachmentData. Also fix the problem of the email body text's missing message ID while
            keeping column MESSAGE_ID's IDENTITY constraint's benefit intact.
****************************************************************************************************************/
...
-- PRINT '*~^**************************************************************************************************************'
-- PRINT '*~^                                                                                                             *'
PRINT '*~^                           BEGIN XX_SEND_STATUS_MAIL'
-- PRINT '*~^                                                                                                             *'
-- PRINT '*~^**************************************************************************************************************'
-- PRINT '' -- *~^ CR9295
...
EXEC @IMAPSReturnCode = dbo.XX_GET_LOOKUP_DATA
   ...
   @usr_app_code           = @InterfaceName, -- CR11604
   @usr_lookup_id          = NULL,
...
SET @email_msg_text = 'This message was generated by the IMAPS financial system resulting from execution of the '
                      ...
                      + @StatusMessage +
                        ...
                        END + 
                        CASE
                           WHEN (@ErrorRecords is NULL) THEN ' all successfuly processed.'
...
-- DR-12027 Begin
PRINT 'Retrieving the new XX_IMAPS_MAIL_OUT record''s column MESSAGE_ID''s value ...'

-- When the XX_IMAPS_MAIL_OUT record is being inserted, the trigger XX_IMAPS_MAIL_OUT_INSTRIG (1) appends MESSAGE_ID's value to MESSAGE_TEXT's value,
-- (2) then calls msdb.dbo.sp_send_dbmail to actually send the mail (this also commits the record), and (3) finally updates MESSAGE_TEXT and SEND_DT.

-- There can be multiple XX_IMAPS_MAIL_OUT records created for a single interface run.
SELECT @msg_id = MESSAGE_ID
  FROM dbo.XX_IMAPS_MAIL_OUT
 WHERE STATUS_RECORD_NUM = @in_StatusRecordNum
   AND MESSAGE_ID = (select MAX(MESSAGE_ID) from dbo.XX_IMAPS_MAIL_OUT where STATUS_RECORD_NUM = @in_StatusRecordNum)
...
-- PRINT convert(varchar, current_timestamp, 21) + ' : *~^ COMMON_CODE : Line 247 : XX_SEND_STATUS_MAIL.sql '  --CR9295
 
UPDATE dbo.XX_IMAPS_MAIL_OUT 
   SET MESSAGE_TEXT = MESSAGE_TEXT + CAST(@@IDENTITY AS VARCHAR(10))
...
PRINT '                           END OF XX_SEND_STATUS_MAIL'
-- DR-12027 End
